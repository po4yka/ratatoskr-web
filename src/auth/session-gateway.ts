import { createGateway, type Gateway, type GatewayDeps } from "@/api/gateway/client"
import {
...
export function wireSessions(
  ...
  // Late binding: the gateway needs the provider for refresh/revocation while
  // the provider needs the gateway to probe through. Both resolve lazily.
  let provider: PresentedCredentialProvider

  const gateway = createGateway({
    ...deps,
    tokenSource: {
      current: () => provider.tokenSource.current(),
    },
